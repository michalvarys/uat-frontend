...
const MultiGallerySlide = ({ galleries, isSmall = false }: Props) => {
  const [currentGallery, setCurrentGallery] = useState<number>(galleries && galleries.length > 0 ? 0 : -1);
  ...
  return (
    <div className={styles.gallery_container}>
      <GalleryMenu
        names={galleries.map((item: GalleryType, idx: number) => item.title || `Gallery #${idx}`)}
